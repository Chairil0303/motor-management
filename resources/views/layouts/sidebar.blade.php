<aside :class="{
        ...
        '-translate-x-full': !$store.sidebar.isOpen,
           ...
           -translate-x-full lg:translate-x-0" x-cloak>

    <!-- Header -->
    <div class="flex items-center justify-between p-4 border-b bg-white">
        <div class="text-xl font-bold text-gray-800">Ken Motor</div>

        <button @click="$store.sidebar.close()" x-show="$store.sidebar.isMobile"
    ...
    <nav class="flex-1 overflow-y-auto py-4">
        <ul class="space-y-1">
            ...
            <!-- Dashboard -->
            <li>
                <a href="{{ route('dashboard') }}" class="flex items-center px-4 py-3 text-gray-700 hover:bg-gray-100"
            ...
            {{-- ===== SHOWROOM ===== --}}
            {{-- ===================== --}}
            @if(auth()->user()->canAccessShowroom())
                <li class="mt-6 mb-2 px-4">
